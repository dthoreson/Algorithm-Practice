...
function musicRuntime(busDuration, songDurations) {
    let targetTime = busDuration - 30;
    let newArr = [];
    for(let i = 0; i < songDurations.length; i++){
        for(let j = i+1; j < songDurations.length; j++){
            if(songDurations[i] + songDurations[j] === targetTime){
                if(newArr.length < 1){
                    newArr = [i,j];
                }else{
                    if(Math.max(songDurations[i], songDurations[j]) > Math.max(songDurations[newArr[0]], songDurations[[0]])){
                        newArr = [i,j];
                    }
                }
            }
